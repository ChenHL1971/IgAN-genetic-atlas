...
box <- function(x, y, w, h, lab, fill = "white", bold = FALSE, size = 2.15)
  list(annotate("rect", xmin = x - w/2, xmax = x + w/2, ymin = y - h/2, ymax = y + h/2, fill = fill, colour = "grey20", linewidth = 0.3),
       annotate("text", x = x, y = y, label = lab, size = size, lineheight = 0.95, family = FAM, fontface = if (bold) "bold" else "plain"))
...
if (capabilities("aqua")) { quartz(type = "pdf", file = file.path(fig_dir, "Fig1_design.pdf"), width = 180/25.4, height = 130/25.4); print(f1); dev.off()
} else ggsave(file.path(fig_dir, "Fig1_design.pdf"), f1, width = 180, height = 130, units = "mm", bg = "white", device = cairo_pdf)
